...
// ---- GATE 2: agent judgment ----
// "cacheable" is the LAST key in the output parser's schema, which makes it the
// first casualty when the model runs out of output budget mid-JSON. The parser
// has autoFix on, so a truncated object does not error — a second LLM call
// repairs it, and it has no way to know what the boolean was meant to be. The
// reply survives (first key), so the bot looks healthy and only caching dies.
//
// "the agent decided false" and "the field never arrived" are two different
// faults with two different fixes, so record which one this was instead of
// collapsing both into false. If cacheableMissing is true in production, the
// agent's model node is truncating — raise or remove its maxOutputTokens.
//
// The string test is not paranoia: a repaired object can come back with "true"
// as text, and === true reads that as false, i.e. the same symptom again.
const cacheableRaw = parsedData.cacheable;
const cacheableMissing = cacheableRaw === undefined || cacheableRaw === null;
const agentCacheable = cacheableRaw === true || cacheableRaw === 'true';
...
return {
    json: {
        ...
        // Which gate said no. storeable false with all three of these benign
        // means the agent genuinely judged the turn uncacheable.
        agentCacheable,
        cacheableMissing,
        bareConfirmation,
        tampered
